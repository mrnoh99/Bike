...
/// 탭하면 BikeComputer 워치 앱을 연다. 데이터는 App Group 공유 저장소에서 읽는다.
...
struct BikeComplicationView: View {
    ...
    var body: some View {
        ...
        case .accessoryCorner:
            Text(hrText)
                ...
                .widgetLabel("BikeComputer \(distText) km")

        case .accessoryRectangular:
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Label("BikeComputer", systemImage: "bicycle")
...
struct BikeComplication: Widget {
    let kind = "BikeComputerComplication"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: RideProvider()) { entry in
            BikeComplicationView(entry: entry)
                .containerBackground(.clear, for: .widget)
        }
        .configurationDisplayName("BikeComputer")
...
@main
struct BikeComputerWatchWidgetBundle: WidgetBundle {
